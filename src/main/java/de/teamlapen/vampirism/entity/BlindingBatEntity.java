...
public class BlindingBatEntity extends Bat {
    ...
    private final TargetingConditions nonVampirePredicatePlayer = TargetingConditions.forCombat().selector((entity, level) -> VampirismAPI.factionRegistry().getPredicate(ModFactions.VAMPIRE, true).and(EntitySelector.NO_CREATIVE_OR_SPECTATOR).test(entity));
    private final TargetingConditions nonVampirePredicate = TargetingConditions.forCombat().selector((e, level) -> !Helper.appearsAsVampire(e, this));
