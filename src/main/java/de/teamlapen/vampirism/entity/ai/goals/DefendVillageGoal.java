...
public class DefendVillageGoal<T extends PathfinderMob & IVillageCaptureEntity> extends TargetGoal {
    ...
    public DefendVillageGoal(@NotNull T creature) {
        ...
        }.selector(VampirismAPI.factionRegistry().getSelector(creature.getFaction(), false)).ignoreInvisibilityTesting();
    }

    @Override
    public boolean canUse() {
        if (!creature.isDefendingVillage()) return false;
        this.targetMob = ((ServerLevel)this.creature.level()).getNearestEntity(LivingEntity.class, entityPredicate, this.mob, this.mob.getX(), this.mob.getY() + (double) this.mob.getEyeHeight(), this.mob.getZ(), creature.getTargetVillageArea());
