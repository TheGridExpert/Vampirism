...
public class DarkBloodProjectileEntity extends AbstractHurtingProjectile {
    ...
    public void explode(int distanceSq, @Nullable Entity excludeEntity) {
        if (this.level() instanceof ServerLevel serverLevel) {
            ...
            if (!this.level().isClientSide) {
                ModParticles.spawnParticlesServer(this.level(), new GenericParticleOption(VResourceLocation.mc("spell_1"), 7, 0xA01010, 0.2F), this.getX(), this.getY(), this.getZ(), 40, 1, 1, 1, 0);
                ModParticles.spawnParticlesServer(this.level(), new GenericParticleOption(VResourceLocation.mc("spell_6"), 10, 0x700505), this.getX(), this.getY(), this.getZ(), 15, 1, 1, 1, 0);
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            ...
            ModParticles.spawnParticlesClient(this.level(), new GenericParticleOption(VResourceLocation.mc("spell_4"), 4, 0xA01010, 0f), center.x, center.y, center.z, 5, getPickRadius(), this.random);

            if (this.tickCount % 3 == 0) {
                ModParticles.spawnParticleClient(this.level(), new GenericParticleOption(VResourceLocation.mc("effect_4"), 12, 0xC01010, 0.4F), center.x, center.y, center.z);
            }
        }
