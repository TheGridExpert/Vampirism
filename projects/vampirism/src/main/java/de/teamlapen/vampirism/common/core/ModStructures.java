...
import de.teamlapen.vampirism.common.world.structures.mother.MotherPiece;
import de.teamlapen.vampirism.common.world.structures.mother.MotherStructure;
import de.teamlapen.vampirism.common.world.structures.placement.SingleOccurrenceStructurePlacement;
import de.teamlapen.vampirism.common.world.structures.templatesystem.BiomeTopBlockProcessor;
import de.teamlapen.vampirism.common.world.structures.templatesystem.RandomBlockStateRule;
import de.teamlapen.vampirism.common.world.structures.templatesystem.RandomCandleLitProcessor;
import de.teamlapen.vampirism.common.world.structures.templatesystem.RandomStructureProcessor;
import de.teamlapen.vampirism.common.world.structures.vampirealtar.VampireAltarPieces;
...
public class ModStructures {
    ...
    public static final DeferredRegister<StructurePlacementType<?>> STRUCTURE_PLACEMENT_TYPES = DeferredRegister.create(Registries.STRUCTURE_PLACEMENT, REFERENCE.MODID);
    ...
    public static final DeferredHolder<StructurePieceType, StructurePieceType> CRYPT_PIECE = STRUCTURE_PIECES.register("crypt", () -> (StructurePieceType) CryptPiece::new);
    ...
    static void createStructureProcessorLists(BootstrapContext<StructureProcessorList> context) {
        context.register(TOTEM_FACTION, new StructureProcessorList(ImmutableList.of(
                new RandomStructureProcessor(ImmutableList.of(new RandomBlockStateRule(new RandomBlockMatchTest(FactionBlocks.TOTEM_TOP.get(), 0.6f), AlwaysTrueTest.INSTANCE, FactionBlocks.TOTEM_TOP.get().defaultBlockState(), TotemTopBlock.getBlocks().stream().filter((totemx) -> totemx != FactionBlocks.TOTEM_TOP.get() && !totemx.isCrafted()).map(Block::defaultBlockState).collect(Collectors.toList())))),
...
//        context.register(HUNTER_CAMP, new HunterCampStructure(new Structure.StructureSettings.Builder(new AndHolderSet<>(List.of(lookup.getOrThrow(ModBiomeTags.HasStructure.HUNTER_TENT), new NotHolderSet<>(context.registryLookup(Registries.BIOME).orElseThrow(),lookup.getOrThrow(ModBiomeTags.IS_FACTION_BIOME))))).terrainAdapation(TerrainAdjustment.BEARD_THIN).build()));
        ...
        context.register(HUNTER_OUTPOST_PLAINS, new JigsawStructure(new Structure.StructureSettings.Builder(lookup.getOrThrow(ModBiomeTags.HasStructure.HUNTER_OUTPOST_PLAINS)).spawnOverrides(Map.of(VEnums.HUNTER_CATEGORY.getValue(), new StructureSpawnOverride(StructureSpawnOverride.BoundingBoxType.PIECE, WeightedList.<MobSpawnSettings.SpawnerData>builder().add(new MobSpawnSettings.SpawnerData(ModEntities.HUNTER.get(), 2, 4), 80).add(new MobSpawnSettings.SpawnerData(ModEntities.ADVANCED_HUNTER.get(), 1, 22), 20).build()))).generationStep(GenerationStep.Decoration.SURFACE_STRUCTURES).terrainAdapation(TerrainAdjustment.BEARD_THIN).build(), lookup1.getOrThrow(PlainsHunterOutpostPools.START), 7, ConstantHeight.of(VerticalAnchor.absolute(0)), true, Heightmap.Types.WORLD_SURFACE_WG));
        context.register(HUNTER_OUTPOST_DESERT, new JigsawStructure(new Structure.StructureSettings.Builder(lookup.getOrThrow(ModBiomeTags.HasStructure.HUNTER_OUTPOST_DESERT)).spawnOverrides(Map.of(VEnums.HUNTER_CATEGORY.getValue(), new StructureSpawnOverride(StructureSpawnOverride.BoundingBoxType.PIECE, WeightedList.<MobSpawnSettings.SpawnerData>builder().add(new MobSpawnSettings.SpawnerData(ModEntities.HUNTER.get(), 2, 4), 80).add(new MobSpawnSettings.SpawnerData(ModEntities.ADVANCED_HUNTER.get(), 1, 22), 20).build()))).generationStep(GenerationStep.Decoration.SURFACE_STRUCTURES).terrainAdapation(TerrainAdjustment.BEARD_THIN).build(), lookup1.getOrThrow(DesertHunterOutpostPools.START), 7, ConstantHeight.of(VerticalAnchor.absolute(0)), true, Heightmap.Types.WORLD_SURFACE_WG));
        context.register(HUNTER_OUTPOST_VAMPIRE_FOREST, new JigsawStructure(new Structure.StructureSettings.Builder(lookup.getOrThrow(ModBiomeTags.HasStructure.HUNTER_OUTPOST_VAMPIRE_FOREST)).spawnOverrides(Map.of(VEnums.HUNTER_CATEGORY.getValue(), new StructureSpawnOverride(StructureSpawnOverride.BoundingBoxType.PIECE, WeightedList.<MobSpawnSettings.SpawnerData>builder().add(new MobSpawnSettings.SpawnerData(ModEntities.HUNTER.get(), 2, 4), 80).add(new MobSpawnSettings.SpawnerData(ModEntities.ADVANCED_HUNTER.get(), 1, 22), 20).build()))).generationStep(GenerationStep.Decoration.SURFACE_STRUCTURES).terrainAdapation(TerrainAdjustment.BEARD_THIN).build(), lookup1.getOrThrow(VampireForestHunterOutpostPools.START), 7, ConstantHeight.of(VerticalAnchor.absolute(0)), true, Heightmap.Types.WORLD_SURFACE_WG));
