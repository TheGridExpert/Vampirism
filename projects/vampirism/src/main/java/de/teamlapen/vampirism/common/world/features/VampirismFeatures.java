...
public class VampirismFeatures {
    ...
    public static void createPlacedFeatures(BootstrapContext<PlacedFeature> context) {
        ...
        context.register(VAMPIRE_FLOWER_PLACED, new PlacedFeature(configuredFeatures.getOrThrow(VAMPIRE_FLOWER), onlyInAir(List.of(RarityFilter.onAverageOnceEvery(2), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP_WORLD_SURFACE, BiomeFilter.biome(), CountPlacement.of(5), RandomOffsetPlacement.ofTriangle(2, 1)))));
        context.register(CURSED_ROOT_PLACED, new PlacedFeature(configuredFeatures.getOrThrow(CURSED_ROOT), onlyInAir(ImmutableList.<PlacementModifier>builder().addAll(VegetationPlacements.worldSurfaceSquaredWithCount(3)).add(CountPlacement.of(6)).add(RandomOffsetPlacement.ofTriangle(3, 1)).build())));
        ...
        context.register(VAMPIRE_FOREST_GRASS_PLACED, new PlacedFeature(configuredFeatures.getOrThrow(VegetationFeatures.GRASS), onlyInAir(VegetationPlacements.worldSurfaceSquaredWithCount(30))));
        context.register(VAMPIRE_FOREST_TALL_GRASS_PLACED, new PlacedFeature(configuredFeatures.getOrThrow(VegetationFeatures.TALL_GRASS), onlyInAir(List.of(NoiseThresholdCountPlacement.of(-0.8, 1, 9), RarityFilter.onAverageOnceEvery(8), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome()))));
    }

    /**
     * Grass and flowers commonly break the floor of crypts and other structures. This makes the plants only get planted where it's empty.
     */
