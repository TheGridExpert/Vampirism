 ...
 * 1) New surface rules. These are added via a Mixin hook ({@link NoiseGeneratorSettingsMixin} on static init. And later on via TerraBlender, if installed
 * 2) The overworld BiomeSource preset is modified on load complete, if TerraBlender is not installed, to include the vampirism forest. If TerraBlender is installed this is done via TerraBlender in common setup in {@link TerraBlenderRegistration}
 */
